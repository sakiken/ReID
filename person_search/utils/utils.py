...
# Plotting functions ---------------------------------------------------------------------------------------------------
def plot_one_box_find(x, img, color=None, label=None, line_thickness=None):  #when find
    # Plots one bounding box on image img
    # tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1     # line thickness 线宽
    # color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = ( 0 , 0), ( 62, 126)                                                     # img_size: 64*128
    # c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))                         #c1 = (x1, y1) = 矩形框的左上角   c2 = (x2, y2) = 矩形框的右下角
    cv2.rectangle(img, c1, c2, (0,255,0), thickness=2)                                 #在im上画出框框
    # if label:                                                                          #文字
...
def plot_one_box_notfind(x, img, color=None, label=None, line_thickness=None):  #when not find
    # Plots one bounding box on image img
    # tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1     # line thickness 线宽
    # color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = ( 0 , 0), ( 62, 126)                                                     # img_size: 64*128
    # c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))                         #c1 = (x1, y1) = 矩形框的左上角   c2 = (x2, y2) = 矩形框的右下角
    cv2.rectangle(img, c1, c2, (0,0,255), thickness=2)                                 #在im上画出框框
    # if label:                                                                          #文字
    #     tf = max(tl - 1, 1)  # font thickness
    #     t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
    #     c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
...
def plot_results(start=0, stop=0):  # from util.util import *; plot_results()
