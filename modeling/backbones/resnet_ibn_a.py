...
class ResNet_IBN(nn.Module):
    ...
    def __init__(self, last_stride, block, layers, num_classes=1000):
        ...
        self.conv1 = nn.Conv2d(3, scale, kernel_size=7, stride=2, padding=3,
                               bias=False) #卷积层，输入通道数为 3，输出通道数为 scale，卷积核大小为 7x7，步长为 2，填充为 3
        self.bn1 = nn.BatchNorm2d(scale)  #批归一化层
        self.relu = nn.ReLU(inplace=True) #激活函数ReLU
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        # #残差块层，通过调用 _make_layer 方法创建，分别使用了不同的输入通道数和输出通道数，以及不同的步长参数。
        self.layer1 = self._make_layer(block, scale, layers[0])
        self.layer2 = self._make_layer(block, scale*2, layers[1], stride=2)
        self.layer3 = self._make_layer(block, scale*4, layers[2], stride=2)
        self.layer4 = self._make_layer(block, scale*8, layers[3], stride=last_stride)
        self.avgpool = nn.AvgPool2d(7)   #平均池化层，核大小为 7x7，用于将特征图的空间维度进行降维
        self.fc = nn.Linear(scale * 8 * block.expansion, num_classes)  #全连接层，将降维后的特征映射到预定义的类别数 num_classes
